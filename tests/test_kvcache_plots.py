...
def test_lineage_requires_realized_cache_read() -> None:
    call = {"cacheRead": 0, "logical_source_candidates": [3]}
...
def test_lineage_size_uses_cache_read_percentage() -> None:
    assert _cache_read_fraction({"input": 200, "cacheRead": 50}) == 0.25
    assert _cache_read_fraction({"input": 0, "cacheRead": 50}) == 0.0
    assert _cache_read_fraction({"input": 100, "cacheRead": 150}) == 1.0
        ...
        "per_call": [
            {
                ...
                "time_since_latest_compatible_prefix_s": float(i + 1),
...
def test_report_embeds_kvcache_figures(
    ...
    assert "**Top 5 slowest calls**" in report
