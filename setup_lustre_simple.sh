#!/bin/bash
#
# Simplified Lustre setup for a fixed 3-node CloudLab layout:
#   - 1 MGS + MDT node
#   - 1 OST node
#   - 1 client node
#
# Just edit the variables below and run: bash setup_lustre_simple.sh
#

set -euo pipefail

# ============================================================
# Node names + filesystem config come from cloudlab_env.sh.
# Update node names there (one place), then:
#     source cloudlab_env.sh
#     bash setup_lustre_simple.sh
# ============================================================
SSH_USER="${SSH_USER:-Minqiu}"
MGS_NODE="${MGS_NODE:?MGS_NODE not set — source cloudlab_env.sh first}"
OST_NODE="${OST_NODE:?OST_NODE not set — source cloudlab_env.sh first}"
CLIENT_NODE="${CLIENT_NODE:?CLIENT_NODE not set — source cloudlab_env.sh first}"
...
# ----------------------------------------------------------------
# 1) Get MGS IP (from CloudLab's /var/emulab/boot/ifmap)
# ----------------------------------------------------------------
...
# 2) Set up MGS + MDT
# ----------------------------------------------------------------
echo "==> Setting up MGS+MDT on $MGS_NODE"
ssh -tt -p 22 "$SSH_USER@$MGS_NODE" << EOF
sudo su -
$FIND_FREE_DISK
